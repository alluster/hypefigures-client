				...
				<div className="space-y-8 lg:grid lg:grid-cols-2 sm:gap-6 xl:gap-10 lg:space-y-0">
					{
						stripePricingPlans.map((item, i) => (
							<div key={i} className="flex flex-col p-6 max-w-lg text-center text-gray-900 bg-white rounded-lg border border-gray-100 shadow dark:border-gray-600 xl:p-8 dark:bg-gray-800 dark:text-white flex-grow">
								<h3 className="mb-4 text-2xl font-semibold">{item.priceTitle}</h3>
								{
									activeSubscription === true && currentPlan === item.priceId
										?
										<p className="font-medium text-green-500 sm:text-lg dark:text-gray-400">This is your team&apos;s active plan</p>
										:
